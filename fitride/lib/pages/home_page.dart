...
class _HomePageState extends State<HomePage> with TickerProviderStateMixin {
  ...
  Future<void> _fetchStreakData(String userId) async {
    ...
    try {
      ...
      DocumentSnapshot<Map<String, dynamic>> streakSnapshot =
      ...
      if (streakSnapshot.exists) {
        ...
        print(
            'Stored streak: $storedStreakCount, Last completed week: $lastCompletedWeek');
      } else {
      ...
      if (currentWeekSnapshot.docs.isNotEmpty) {
        currentWeekData =
            currentWeekSnapshot.docs.first.data() as Map<String, dynamic>;
        ...
        double completionPercentage =
            daysTarget > 0 ? (daysProgress / daysTarget) : 0.0;
        currentWeekCompleted = completionPercentage >= 1.0;

        print(
            'Current week progress: $daysProgress/$daysTarget (${(completionPercentage * 100).toStringAsFixed(1)}%)');
      ...
      if (previousWeekSnapshot.docs.isNotEmpty) {
        Map<String, dynamic> previousWeekData =
            previousWeekSnapshot.docs.first.data() as Map<String, dynamic>;
        ...
        if (previousWeekData.containsKey('targetDaysPerWeek')) {
          daysTarget =
              (previousWeekData['targetDaysPerWeek'] as num).toDouble();
        } else if (goalData.containsKey('daysPerWeek')) {
          daysTarget = (goalData['daysPerWeek'] as num).toDouble();
        }
      ...
      // CASE 2: Current week is incomplete, but previous week was complete and hasn't been counted
          ...
          lastCompletedWeek != currentWeekNumber - 1) {
        newStreakCount = (storedStreakCount ?? 0) + 1;
        newLastCompletedWeek = currentWeekNumber - 1;
        shouldPlayAnimation = true;
        print(
            'INCREMENTING STREAK: Previous week complete and not yet counted');
      }
      ...
      if (newStreakCount != storedStreakCount ||
          newLastCompletedWeek != lastCompletedWeek) {
        if (streakSnapshot.exists) {
          await FirebaseFirestore.instance
              .collection('Streak')
              .doc(userId)
          ...
          print(
              'Updated streak document: streak=$newStreakCount, lastCompletedWeek=$newLastCompletedWeek');
        } else {
          await FirebaseFirestore.instance
              .collection('Streak')
              .doc(userId)
          ...
          print(
              'Created new streak document: streak=$newStreakCount, lastCompletedWeek=$newLastCompletedWeek');
        }
      }
  ...
  Future<Map<String, dynamic>> _fetchCurrentWeekProgress() async {
    ...
    try {
      ...
      Map<String, dynamic> weekData =
          weeklyProgressSnapshot.docs.first.data() as Map<String, dynamic>;
      ...
      double progress =
          daysTarget > 0 ? (daysProgress / daysTarget).clamp(0.0, 1.0) : 0.0;
                            ...
                            TweenAnimationBuilder<double>(
                              ...
                              child: Container(
                                ...
                                decoration: BoxDecoration(
                                  ...
                                  color:
                                      Colors.black, // Black circle background
                                  boxShadow: [
                                    BoxShadow(
            ...
            Stack(
              children: [
                ...
                AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    color: Colors
                        .black, // Black filled portion instead of gradient
                    borderRadius: BorderRadius.circular(10),
                    boxShadow: [
                      BoxShadow(
                ...
                ...List.generate(daysTarget, (index) {
                  double markerPosition = MediaQuery.of(context).size.width *
                      0.8 *
                      (index + 1) /
                      daysTarget;
                  return Positioned(
        ...
        children: [
          ...
          if (bodyMetricsData.isNotEmpty)
            Padding(
              ...
              child: _buildBodyMetricsCard(bodyMetricsData[0],
                  _formatMetricsDate(bodyMetricsData[0]['timestamp']), true),
            ),
  ...
  Widget _buildBodyMetricsCard(
      Map<String, dynamic> metricsData, String dateLabel, bool isLatest) {
    // Get metrics values
  ...
  Future<void> _fetchFoodDiaryData() async {
    ...
    try {
      ...
      print(
          "Fetching meals from ${oneWeekAgo.toString()} to ${today.toString()}");
        ...
        children: [
          ...
          if (nutritionData.isNotEmpty)
            Padding(
              padding:
                  const EdgeInsets.symmetric(horizontal: 16.0, vertical: 16.0),
              child: _buildEnhancedMealCard(nutritionData[0],
                  _formatMealDate(nutritionData[0]['date']), true),
            ),
              ...
              child: Column(
                children: [
                  for (int i = 1; i < nutritionData.length; i++)
                    Padding(
                      padding: const EdgeInsets.only(top: 16.0),
                      child: _buildEnhancedMealCard(nutritionData[i],
                          _formatMealDate(nutritionData[i]['date']), false),
                    ),
                ],
              ),
            ),
  ...
  Widget _buildEnhancedMealCard(
      Map<String, dynamic> dayData, String dayLabel, bool isToday) {
    ...
    final totalCalories =
        double.tryParse(dayData['total_calories'].toString()) ?? 0;
    final totalProtein =
        double.tryParse(dayData['total_protein'].toString()) ?? 0;
                  ...
                  children: [
                    if (hasBreakfast)
                      _buildMealItem(
                        ...
                        calories:
                            "${double.tryParse(dayData['breakfast_calories'].toStringAsFixed(2)) ?? 0}",
                        protein:
                            "${double.tryParse(dayData['breakfast_protein'].toStringAsFixed(2)) ?? 0}",
                        carbs:
                            "${double.tryParse(dayData['breakfast_carbs'].toStringAsFixed(2)) ?? 0}",
                        fat:
                            "${double.tryParse(dayData['breakfast_fat'].toStringAsFixed(2)) ?? 0}",
                        color: Color(0xffFFA500),
                    ...
                    if (hasLunch)
                      _buildMealItem(
                        ...
                        calories:
                            "${double.tryParse(dayData['lunch_calories'].toStringAsFixed(2)) ?? 0}",
                        protein:
                            "${double.tryParse(dayData['lunch_protein'].toStringAsFixed(2)) ?? 0}",
                        carbs:
                            "${double.tryParse(dayData['lunch_carbs'].toStringAsFixed(2)) ?? 0}",
                        fat:
                            "${double.tryParse(dayData['lunch_fat'].toStringAsFixed(2)) ?? 0}",
                    ...
                    if (hasDinner)
                      _buildMealItem(
                        ...
                        calories:
                            "${double.tryParse(dayData['dinner_calories'].toStringAsFixed(2)) ?? 0}",
                        protein:
                            "${double.tryParse(dayData['dinner_protein'].toStringAsFixed(2)) ?? 0}",
                        carbs:
                            "${double.tryParse(dayData['dinner_carbs'].toStringAsFixed(2)) ?? 0}",
                        fat:
                            "${double.tryParse(dayData['dinner_fat'].toStringAsFixed(2)) ?? 0}",
